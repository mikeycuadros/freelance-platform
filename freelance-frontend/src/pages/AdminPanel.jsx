  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-20">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 ${activeTab === "categories" ? "border-b-2 border-purple-800 text-purple-800" : "text-gray-500"}`}
          onClick={() => setActiveTab("categories")}
        >
          Gestión de Categorías
        </button>
        <button
          className={`py-2 px-4 ${activeTab === "stats" ? "border-b-2 border-purple-800 text-purple-800" : "text-gray-500"}`}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">Icono</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Descripción</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {loading ? (
                  <tr>
                    <td colSpan="4" className="px-4 py-4 text-center">Cargando...</td>
                  </tr>
                ) : categories.length === 0 ? (
                  <tr>
                    <td colSpan="4" className="px-4 py-4 text-center">No hay categorías disponibles</td>
                  </tr>
                ) : (
                  categories.map((category) => (
                    <tr key={category.id}>
                      <td className="px-4 py-4 whitespace-nowrap text-sm">{category.name}</td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm hidden sm:table-cell">{category.icon}</td>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-3">Usuarios con más mensajes</h3>
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-3">Usuarios con más reseñas</h3>
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            <h3 className="text-lg font-semibold mb-3">Usuarios con mejor rating</h3>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <Tooltip 
                        ...
                        formatter={(value) => [parseFloat(value).toFixed(1), 'Calificación']}
                      />
                      <Legend wrapperStyle={{ paddingTop: '10px' }} />
                      <Bar 
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                      <Pie
                        ...
                        cy="50%"
                        labelLine={true}
                        label={({ name, value, percent }) => `${name}: ${parseFloat(value).toFixed(1)} (${(percent * 100).toFixed(0)}%)`}
                        outerRadius={80}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-3">Top usuarios por mensajes</h3>
              {stats.topMessageReceivers.length === 0 ? (
                <p className="text-gray-500">No hay datos disponibles</p>
              ) : (
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-3">Top usuarios por reseñas</h3>
              {stats.topReviewReceivers.length === 0 ? (
                <p className="text-gray-500">No hay datos disponibles</p>
              ) : (
                <ul className="space-y-2">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
              <h3 className="text-lg font-semibold mb-3">Top usuarios por rating</h3>
              {stats.topRatedFreelancers.length === 0 ? (
                <p className="text-gray-500">No hay datos disponibles</p>
              ) : (
                <ul className="space-y-2">
                  {stats.topRatedFreelancers.map((user, index) => (
                    <li key={user.id} className="flex justify-between items-center">
                      ...
                      <div className="flex items-center">
                        ...
                        <span>{parseFloat(user.averageRating).toFixed(1)}</span>
                      </div>
                    </li>
                  ))}
                </ul>
